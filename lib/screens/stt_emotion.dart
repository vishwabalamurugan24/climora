...
class _SttEmotionScreenState extends State<SttEmotionScreen> {
  ...
  void _start() {
    ...
    _speech.startListeningMultilingual(onResult: (text, level) async {
      setState(() {
        _transcript = text;
        _soundSamples++;
        _avgSoundLevel = ((_avgSoundLevel * (_soundSamples - 1)) + level) / _soundSamples;
      });
  ...
  Future<void> _playRecommendation() async {
    ...
    final reco = _reco.recommend(emotion: _emotionResult, vocalLevel: _avgSoundLevel);
    // last recommendation intentionally not stored in UI state
    // load and play music
    final musicKey = 'music_track';
    try {
      if (!_audio.isLoaded(musicKey)) await _audio.loadAsset(musicKey, reco.songAsset, volume: reco.volume);
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Text(_transcript.isEmpty ? '(no speech yet)' : _transcript),
            ),
            const SizedBox(height: 16),
            Text('Vocal-tone proxy (avg sound level): ${_avgSoundLevel.toStringAsFixed(2)}'),
            const SizedBox(height: 12),
            Text('Semantic emotion:'),
            const SizedBox(height: 8),
            if (_emotionResult == null) const Text('(no result)') else Text('${_emotionResult!.label} (${_emotionResult!.score.toStringAsFixed(2)})'),
            const SizedBox(height: 12),
            ElevatedButton(
              onPressed: _emotionResult == null ? null : _playRecommendation,
              child: Text(_musicPlaying ? 'Stop Recommendation' : 'Play Recommendation'),
            ),
            const SizedBox(height: 8),
            ElevatedButton(onPressed: _stopAllAudio, child: const Text('Stop All Audio')),
            const SizedBox(height: 8),
            ElevatedButton(onPressed: _mapLocalMusic, child: const Text('Map Local Music')),
          ],
        ),
      ),
    );
