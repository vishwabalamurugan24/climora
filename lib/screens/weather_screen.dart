          ...
          children: [
            Row(
              children: [
                ...
                Text(_settings.enabled ? 'Weather-based Song Mode: ON' : 'Weather-based Song Mode: OFF'),
                const Spacer(),
                IconButton(
            ...
            const Divider(),
              void _showCustomizationPanel(BuildContext context) async {
                        ...
                        return Padding(
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('Customize Weather-based Music', style: TextStyle(fontWeight: FontWeight.bold)),
                                const SizedBox(height: 12),
                                Text('Preferred Time Ranges:'),
                                Wrap(
                                  spacing: 8,
                                  children: TimeRange.values.map((tr) => FilterChip(
                                    label: Text(tr.name),
                                    selected: temp.timeRanges.contains(tr),
                                    onSelected: (v) => setSheetState(() => temp = temp.copyWith(
                                      timeRanges: v ? {...temp.timeRanges, tr} : {...temp.timeRanges}..remove(tr),
                                    )),
                                  )).toList(),
                                ),
                                const SizedBox(height: 12),
                                ...
                                Wrap(
                                  spacing: 8,
                                  children: WeatherTrigger.values.map((wt) => FilterChip(
                                    label: Text(wt.name),
                                    selected: temp.weatherTriggers.contains(wt),
                                    onSelected: (v) => setSheetState(() => temp = temp.copyWith(
                                      weatherTriggers: v ? {...temp.weatherTriggers, wt} : {...temp.weatherTriggers}..remove(wt),
                                    )),
                                  )).toList(),
                                ),
                                const SizedBox(height: 12),
                                ...
                                Wrap(
                                  spacing: 8,
                                  children: ['English', 'Tamil', 'Hindi'].map((lang) => FilterChip(
                                    label: Text(lang),
                                    selected: temp.languages.contains(lang),
                                    onSelected: (v) => setSheetState(() => temp = temp.copyWith(
                                      languages: v ? {...temp.languages, lang} : {...temp.languages}..remove(lang),
                                    )),
                                  )).toList(),
                                ),
                                const SizedBox(height: 12),
                                ...
                                Wrap(
                                  spacing: 8,
                                  children: ['lo-fi', 'melody', 'classical', 'energetic', 'devotional'].map((g) => FilterChip(
                                    label: Text(g),
                                    selected: temp.genres.contains(g),
                                    onSelected: (v) => setSheetState(() => temp = temp.copyWith(
                                      genres: v ? {...temp.genres, g} : {...temp.genres}..remove(g),
                                    )),
                                  )).toList(),
                                ),
                                const SizedBox(height: 12),
                                Text('Auto-play Duration:'),
                                DropdownButton<Duration>(
                                  ...
                                  items: [
                                    ...
                                    value: d,
                                    child: Text(d.inMinutes >= 60 ? '${d.inHours} hour' : '${d.inMinutes} min'),
                                  )).toList(),
                                  onChanged: (d) => setSheetState(() => temp = temp.copyWith(autoPlayDuration: d)),
                                ),
                                const SizedBox(height: 12),
                                SwitchListTile(
                                  value: temp.smartNotifications,
                                  onChanged: (v) => setSheetState(() => temp = temp.copyWith(smartNotifications: v)),
                                  title: const Text('Enable smart notifications for mood-based suggestions'),
                                ),
            ...
            ElevatedButton(onPressed: _load, child: const Text('Get current weather')),
            const SizedBox(height: 16),
            if (_loading) const Center(child: CircularProgressIndicator()),
            if (_error != null) Text('Error: $_error', style: const TextStyle(color: Colors.red)),
            if (_weather != null) ...[
              if (_lastUpdated != null) Text('Last updated: ${_lastUpdated!.toLocal()} ($_dataSource)'),
              ...
              const SizedBox(height: 12),
              Text('Comfort index: ${ClimateIndices.comfortIndex(_weather!.temp, _weather!.humidity, _weather!.windSpeed)}'),
              const SizedBox(height: 8),
              Text('Suggested ambient sounds:'),
              const SizedBox(height: 6),
              Wrap(
                spacing: 8,
                children: _suggestions.map((s) => Chip(label: Text(s))).toList(),
              ),
              const SizedBox(height: 12),
              Row(
                children: [
                  ElevatedButton(
                      ...
                      if (!_playing) {
                        ...
                        for (final s in _suggestions) {
                          ...
                          if (!_audio.isLoaded(s)) {
                            try {
                              await _audio.loadAsset(s, path, volume: _volumes[s] ?? 0.6);
                            } catch (_) {
                    ...
                    child: Text(_playing ? 'Stop Ambient Mix' : 'Play Ambient Mix'),
                  ),
                  const SizedBox(width: 12),
                  ElevatedButton(
                    onPressed: () async {
                ...
                children: [
                  ...
                  Slider(
                    ...
                    onChanged: (v) async {
                      ...
                      if (_audio.isLoaded(s)) await _audio.setVolume(s, v);
                    },
                  ),
                ],
              )).toList(),
